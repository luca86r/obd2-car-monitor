...
void BluetoothManager::discoverDevice() {
  ...
  DBG_PRINTLN("BT - Starting discoverAsync...");

  BTScanResults* btDeviceList = serialBT.getScanResults();  // maybe accessing from different threads!
  if (serialBT.discoverAsync([](BTAdvertisedDevice* pDevice) {
      // BTAdvertisedDeviceSet*set = reinterpret_cast<BTAdvertisedDeviceSet*>(pDevice);
      // btDeviceList[pDevice->getAddress()] = * set;
      DBG_PRINTF("BT - >>>>>>>>>>>Found a new device asynchronously: %s\n", pDevice->toString().c_str());
    } )
    ) {
    delay(BT_DISCOVER_TIME);

    DBG_PRINT("BT - Stopping discoverAsync... ");
    serialBT.discoverAsyncStop();
    DBG_PRINTLN("BT - discoverAsync stopped");
    delay(5000);
    ...
    if(btDeviceList != NULL && btDeviceList->getCount() > 0) {
      ...
      for (int i=0; i < btDeviceList->getCount(); i++) {
        ...
        if (!deviceName.equals(OBD_BT_DEVICE_NAME)) {
          DBG_PRINTF("BT ----- %s  %s %d [SKIPPING]\n", device->getAddress().toString().c_str(), device->getName().c_str(), device->getRSSI());
          continue;
        }

        DBG_PRINTF("BT ----- %s  %s %d [FOUND]\n", device->getAddress().toString().c_str(), device->getName().c_str(), device->getRSSI());
        std::map<int,std::string> channels=serialBT.getChannels(device->getAddress());
        DBG_PRINTF("BT - scanned for services, found %d\n", channels.size());
        for(auto const &entry : channels) {
          DBG_PRINTF("BT -      channel %d (%s)\n", entry.first, entry.second.c_str());
        }
      ...
      if(addr) {
        DBG_PRINTF("BT - connecting to %s - %d\n", addr.toString().c_str(), channel);

        if (serialBT.connect(addr, channel, bt_sec_mask, bt_role)) {
          DBG_PRINTLN("BT - Connected!; Saving MAC address" + addr.toString());
      ...
      DBG_PRINTLN("BT - Didn't find any devices");
    }
  } else {
    DBG_PRINTLN("BT - Error on discoverAsync f.e. not workin after a \"connect\"");
  }
}
...
void BluetoothManager::connectBySavedMAC() {
  ...
  if (pairedDeviceAddress != "") {
    DBG_PRINTLN("BT - MAC Address salvato: " + pairedDeviceAddress);
    DBG_PRINTLN("BT - Connetting by MAC...");

    BTAddress addr = BTAddress(pairedDeviceAddress);

    if (serialBT.connect(addr)) {
        DBG_PRINTLN("BT - Connected by MAC!");
    }
    else {
      DBG_PRINTLN("BT - Couldn't connect to BT OBD scanner by MAC - Phase 1");
    }
  }
}